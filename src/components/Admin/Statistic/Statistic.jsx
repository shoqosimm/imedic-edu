...
const StatisticList = () => {
    ...
    const [subject,
        setSubject] = useState([])
    const [categoryId,
        setCategoryId] = useState('')
    const [courseId,
        setCourseId] = useState('')
    const [subjectId,
        setSubjectId] = useState('')
    const [nurses,
        setNurses] = useState(null)
    const [theme,
        setTheme] = useState(null)
    const [testId,
        setTestId] = useState('')
    const [listTest,
        setListTest] = useState([])
    const [show,
...
//Umumiy statistika
    useEffect(() => {
        api.get(`api/admin/statistic/subject/list/${courseId}`) //mavzular listi
            .then((res) => {
    ...
    useEffect(() => {
        ...
        api.get(`api/admin/category/list`) //category list
            .then((res) => {
                if (res.status == 200) {
                    setCategory(res.data.data.map((item) => {
...
setNurseId(value)
    ...
    <Form autoComplete="false" layout="vertical" form={form} id="statictik">
        <Row gutter={[20, 20]} className="d-flex align-end">
            <Col xl={7} lg={7} md={24} sm={24} xs={24}>
                <Form.Item
                    ...
                    <Select options={category} onChange={categoryChange}/>
                </Form.Item>
            </Col>
            <Col xl={7} lg={7} md={24} sm={24} xs={24}>
                <Form.Item
                    ...
                    <Select options={courseList} onChange={courseChange}/>
                </Form.Item>
            </Col>
            <Col xl={7} lg={7} md={24} sm={24} xs={24}>
                <Form.Item
                    ...
                    <Select options={subject} onChange={subjectChange}/>
                </Form.Item>
            </Col>
        </Row>
    </Form>
                    ...
                    <Card
                      ...
                      >
                        {nurseList.map(item=>{
                          return(
                            <li> 
                            <Button ghost onClick={()=>handleList(item.id)}
                            className='d-flex gap-3' 
                            style={{color:'black'}}>
                             <span>{item.id}</span>{item.name}<span></span>
                            </Button>
                            </li>
                          )
                        })}
                            ...
                            <Col span={12}>
                                <Card className='testDesgin'>
                                    <Statistic
                                        ...
                                        prefix={<AiOutlineCheckCircle size={30}/>}/>
                                </Card>
                            </Col>
                            <Col span={12}>
                                <Card className='testDesgin'>
                                    <Statistic
                                        ...
                                        prefix={<AiOutlineCloseCircle size = {
                                        30
                                    } />}/>
                                </Card>
                            </Col>
